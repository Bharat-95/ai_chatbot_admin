...
interface SubscriptionPlan {
  id: string;
  plan_name: string;
  amount_usd: string; // stored as text/varchar in DB
  amount_ind: string; // stored as text/varchar in DB
...
export default function SubscriptionPlansPage() {
  ...
  const fetchPlans = async () => {
    ...
    const { data, error } = await supabaseBrowser
      .from("subscription")
      .select("id, plan_name, amount_usd, amount_ind, type, product_id, price_id, sort_order")
  ...
  // ✅ save (insert or update) — uses amount_usd & amount_ind correctly
  const handleSave = async (plan: SubscriptionPlan) => {
    const payload = {
      plan_name: plan.plan_name,
      amount_usd: plan.amount_usd ?? "",
      amount_ind: plan.amount_ind ?? "",
      type: plan.type,
    ...
    if (plan.id) {
      ...
      // insert
      result = await supabaseBrowser.from("subscription").insert([payload]);
    }
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-gray-50 text-gray-700">
              <tr>
                ...
                <th className="px-4 py-3">USD Amount</th>
                <th className="px-4 py-3">India Amount</th>
            ...
            <tbody>
              {plans.map((plan) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">${plan.amount_usd || "0"}</td>
                  <td className="px-4 py-3">₹{plan.amount_ind || "0"}</td>
        ...
        <DialogContent>
          ...
          {editingPlan && (
            <div className="grid gap-4 py-4">
              ...
              <Input
                type="text"
                value={editingPlan.amount_usd}
                onChange={(e) =>
                  setEditingPlan({ ...editingPlan, amount_usd: e.target.value })
                }
              ...
              <Input
                type="text"
                value={editingPlan.amount_ind}
                onChange={(e) =>
                  setEditingPlan({ ...editingPlan, amount_ind: e.target.value })
                }
              ...
              <Input
                ...
                onChange={(e) =>
                  setEditingPlan({ ...editingPlan, product_id: e.target.value })
                }
              ...
              <Input
                ...
                onChange={(e) =>
                  setEditingPlan({ ...editingPlan, price_id: e.target.value })
                }
          ...
          <DialogFooter>
            ...
            <Button onClick={() => editingPlan && handleSave(editingPlan)}>
              Save
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent>
          ...
          <p className="text-sm text-gray-600">
            ...
            <span className="font-semibold">
              {planToDelete?.plan_name || "this plan"}
            </span>
